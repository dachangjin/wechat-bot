...
const typeParser = async (padLocalMessage, ret, _context) => {
    const wechatMessageType = padLocalMessage.type;
    let type = TypeMappings[wechatMessageType];
    if (!type) {
        wechaty_puppet_1.log.verbose(message_parser_js_1.LOGPRE, `unsupported type: ${JSON.stringify(padLocalMessage)}`);
