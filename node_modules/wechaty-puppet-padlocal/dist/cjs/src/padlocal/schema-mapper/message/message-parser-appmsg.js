...
const appMsgParser = async (padLocalMessage, ret, context) => {
    if (ret.type !== PUPPET.types.Message.Attachment) {
        return ret;
    }
    try {
        const appPayload = await (0, message_appmsg_js_1.parseAppmsgMessagePayload)(padLocalMessage.content);
        ...
        wechaty_puppet_1.log.warn(message_parser_js_1.LOGPRE, `Error occurred while parse message attachment: ${JSON.stringify(padLocalMessage)} , ${e.stack}`);
    }
    return ret;
};
