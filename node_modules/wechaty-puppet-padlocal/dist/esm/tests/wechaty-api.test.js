...
describe("contact", () => {
    test("set self name", async () => {
        ...
        const toName = config.get("test.contact.changeNickName");
    ...
    test("set self signature", async () => {
        const toSignature = config.get("test.contact.changeSignature");
        const self = bot.currentUser;
        await self.signature(toSignature);
    });
    test("set other contact alias", async () => {
        const userName = config.get("test.contact.alias.userName");
        const toAlias = config.get("test.contact.alias.aliasName");
    ...
    test("contact avatar", async () => {
        ...
        const userName = config.get("test.contact.getAvatarUserName");
        const contact = (await bot.Contact.find({ id: userName }));
    ...
    test("delete contact", async () => {
        const deleteUserName = config.get("test.contact.deleteUserName");
...
describe("tag", () => {
    const userName = config.get("test.tag.targetUserName");
    const tagName = config.get("test.tag.addDeleteTagName");
...
describe("friendship", () => {
    const hello = config.get("test.friendship.add.hello");
    test("accept", async () => {
        const friendshipId = config.get("test.friendship.acceptId");
        const friendship = await bot.Friendship.load(friendshipId);
        await friendship.ready();
        await friendship.accept();
    });
    test("add", async () => {
        const userName = config.get("test.friendship.add.userName");
        const contact = await bot.Contact.find({ id: userName });
        expect(contact).toBeTruthy();
        await bot.Friendship.add(contact, hello);
    });
    test("search phone ", async () => {
        const searchPhone = config.get("test.friendship.search.phone");
        const contact = await bot.Friendship.search({ phone: searchPhone });
        expect(contact).toBeTruthy();
        await bot.Friendship.add(contact, hello);
    });
    test("search weixin ", async () => {
        const searchWeixin = config.get("test.friendship.search.weixin");
        const contact = await bot.Friendship.search({ weixin: searchWeixin });
        expect(contact).toBeTruthy();
        await bot.Friendship.add(contact, hello);
    });
});
const toChatRoomId = config.get("test.message.send.chatroomId");
const toUserName = config.get("test.message.send.toUserName");
...
describe("message", () => {
    ...
    test("send text message with at user list", async () => {
        const atUserList = config.get("test.message.send.chatroomAtUserList");
        const text = `hello padlocal: ${Date.now()}`;
        const contactList = [];
    ...
    const sendContactCardMessage = async () => {
        const contactCardId = config.get("test.message.send.contactCardId");
    ...
    const sendImageMessage = async () => {
        const imageFilePath = config.get("test.message.send.imageFilePath");
    ...
    const sendVoiceMessage = async () => {
        const voiceFilePath = config.get("test.message.send.voiceFilePath");
        const voiceLength = config.get("test.message.send.voiceLength");
    ...
    const sendVideoMessage = async () => {
        const videoFilePath = config.get("test.message.send.videoFilePath");
    ...
    const sendFileMessage = async () => {
        const fileFilePath = config.get("test.message.send.fileFilePath");
    ...
    const sendLinkMessage = async () => {
        const title = config.get("test.message.send.link.title");
        const description = config.get("test.message.send.link.description");
        const url = config.get("test.message.send.link.url");
        const thumbImageUrl = config.get("test.message.send.link.thumbImageUrl");
    ...
    const sendMiniProgramMessageThumbCdn = async () => {
        const miniProgramPayload = config.get("test.message.send.miniProgram");
    ...
    const sendMiniProgramMessageThumbHttp = async () => {
        const miniProgramPayload = Object.assign({}, config.get("test.message.send.miniProgram"));
    ...
    const sendEmojiMessage = async () => {
        const emotionPayload = config.get("test.message.send.emoji");
...
describe("room", () => {
    const chatroomId = config.get("test.room.chatroomId");
    test("create room", async () => {
        const memberList = config.get("test.room.create.memberUserNameList");
        const roomName = config.get("test.room.create.roomName");
        const contactList = [];
    ...
    test("room delete member", async () => {
        const memberUserName = config.get("test.room.delete.memberUserName");
        const room = (await bot.Room.find({ id: chatroomId }));
    ...
    test("room add member", async () => {
        const room = (await bot.Room.find({ id: chatroomId }));
        const userName = config.get("test.room.add.memberUserName");
        const contact = await bot.Contact.find({ id: userName });
        const oldMemberList = await room.memberAll();
    ...
    test("room topic", async () => {
        const room = (await bot.Room.find({ id: chatroomId }));
        const toName = config.get("test.room.topic.changeName");
    ...
    test("room announce", async () => {
        const room = (await bot.Room.find({ id: chatroomId }));
        const newAnnouncement = config.get("test.room.announce.newAnnouncement");
    ...
    test("room quit", async () => {
        const roomId = config.get("test.room.quit.id");
        const room = (await bot.Room.find({ id: roomId }));
        await room.quit();
    });
    test("accept room invitation", async () => {
        const roomInvitationId = config.get("test.room.invitation.id");
