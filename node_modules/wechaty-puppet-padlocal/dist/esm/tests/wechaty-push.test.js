...
test("push", async () => {
    ...
    const forwardTo = config.get("test.push.forwardTo");
    const recallUserId = config.get("test.push.recallUserId");
    ...
    await prepareSingedOnBot(async (bot) => {
        ...
        bot.on("room-join", async (room, inviteeList, inviter, date) => {
            log.info(LOGPRE, `on room join: ${room.toString()}, inviteeList: ${inviteeList.map((i) => i.id)}, inviter: ${inviter.id}, ${date}`);
        });
        bot.on("room-leave", async (room, leaverList, remover, date) => {
            log.info(LOGPRE, `on room leave: ${room.toString()}, leaverList: ${leaverList.map((l) => l.id)}, remover: ${remover?.id} ${date}`);
        });
